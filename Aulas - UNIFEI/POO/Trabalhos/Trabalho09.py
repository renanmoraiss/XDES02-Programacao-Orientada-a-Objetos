...
class View():
    def __init__(self, master, controller):
        ...
        self.buttonSearch = tk.Button(self.janela, text="Consultar")
...
class Controller():       
    ...
    def searchHandler(self, event):
        codigoCliente = simpledialog.askstring("Consulta", "Código do cliente:")
        if not codigoCliente:
            return
        for cliente in self.listaClientes:
            if cliente.codigo == codigoCliente:
                msgInfo = "Nome: {}\nEmail: {}".format(cliente.nome, cliente.email)
                self.view.mostraJanela("Cliente encontrado", msgInfo)
                return
